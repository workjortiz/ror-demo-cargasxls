class ProductsController < ApplicationController
  ...
  def import
    if params[:file].present?
      ...
      redirect_to products_load_data_result_path(:instance_load => instance_load), notice: "Proceso de carga Excel carga completado"
    else
  ...
  def index
    ...
    @products = Product.where("description LIKE '%#{@filter}%' OR long_code LIKE '%#{@filter}%' ")
  ...
  def load_data_index
    @bit_history = BitLoadData.select("DISTINCT data1").where(event: 'LOAD_EXCEL', model: 'PRODUCTS').order("data1 DESC").paginate(page: params[:page], per_page: 10)
  end
